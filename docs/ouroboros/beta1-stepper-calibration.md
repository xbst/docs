...
You can also experiment with biquad values live (no restart required) using `SET_TMC_BIQUAD_FILTER` — see the [Plugin Reference](./plugin-reference/) page. Change your biquad filter values in the config, save then `FIRMWARE_RESTART` once you've found values you like.
